...
lm.contribution.ggplot <- function (lm.contribution.object, date.index = NULL, dependent.var = NULL) {
  ...
  if (m.out[variable != dependent.var, min(value)] >= 0) {
    ...
    lm.plot <- ggplot2::ggplot() +
      ggplot2::geom_area(data = m.out[value >= 0 & variable != dependent.var], ggplot2::aes_string(x = date.index, y = "value", fill = "variable")) +
      ggplot2::geom_area(data = m.out[value < 0 & variable != dependent.var], ggplot2::aes_string(x = date.index, y = "value", fill = "variable")) +
      ggplot2::geom_line(data = m.out[variable == dependent.var], ggplot2::aes_string(x = date.index, y = "value"), colour= "black")
  }
